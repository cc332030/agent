...
MECHANISMS = [
    ...
    ("最高关注项不得被删或降级（P1/P2/P3/P5 条款 L1、P4 条款 L2、同列最高关注项）", "specs/core/priority.adoc", "script/check_specs.py", "check_priority_guard 钉住 P1-P5 各自级别、『不可降级』声明与 P1/P4/P5 的必加载层落点；『条目级别是否与其实际后果相符』无机械抓手（等级越高验证越严：ISO/IEC Directives Part 2），由人/子 agent 复核承担"),
    ("定级口径（定级四问 + 条款类型判定表）不得被删", "specs/general/spec-lifecycle.adoc", "script/check_specs.py", "check_spec_admission_guard 钉住『如何给一条规范定级』『与条款类型一一对应』『归属谁』『归类举证』『级别变更与复盘』五个节仍在（口径被删则级别重新混乱）"),
    ("定级方法论与元规范不得涨回常驻层（常驻层只放 L1 底线与最高关注项）", "specs/core/priority.adoc", "script/check_specs.py", "check_priority_guard 拦住常驻层再次出现『设级别』等定级方法论节；『常驻层体量是否反弹』属体量判断，交人/子 agent 复核"),
    ("规范集合自身重构须按固定顺序（先判归属 → 再判层级 → 再判重复 → 压缩表述）", "specs/general/spec-lifecycle.adoc", "script/check_specs.py", "check_spec_admission_guard 钉住重构顺序三要点仍在（顺序颠倒会把放错位置的内容直接删掉）"),
    ("同一条规则的两种读法（执行侧只给'怎么走'、依据与取舍归思考/决策侧）", "specs/general/spec-lifecycle.adoc", "script/check_specs.py", "check_spec_admission_guard 钉住「同一条规则有两种读法」节；常驻层侧由 check_priority_guard 钉住『怎么走』形态声明与各最高关注项的『依据』行（依据不得被整段删掉）；一句话里是否真的没夹解释属文风判断，交人/子 agent 复核"),
