...
MECHANISMS = [
    ...
    ("不得新增只做条件取值的方法（没有现成工具方法就加一个通用方法、不就地抽专用方法）",
     ...
     "check_ternary_extraction_guard 按**条目/行自己的正文**核（不是整节）：它与相邻的「三元表达式为最低优先级」「判空/默认值兜底优先用工具方法」在同节相邻、字样相近，按整节核时会把抽走的要点兜住。"
     "钉住三处判据本体：① 新条「不得新增只做条件取值的方法」——**不得让方法体只剩一处条件取值**、核心理由（没有自己的语义）、判定标准三条（含『把本条读成禁止所有取值方法』的反面）、"
    ...
    ("Maven 既有的构建并行度配置以配置为准，没配过时默认按核心数启用多线程构建（取值以命令行参数形态显式传）",
     ...
     "既有配置优先（`.mvn/maven.config` 为项目级落点、不得覆盖不得重复追加；命令行总是在项目配置文件之后生效，"
     "故再传一次 `-T` 即覆盖）、**取值必须以命令行参数形态显式传**（不得改用 `MAVEN_OPTS` 一类环境变量载体）、并行只到模块粒度、"
    ...
    ("成员与方法次序（新增成员按流程次序落点、可插前面或中间、重载相邻、数据转换方法跟随查询；只对新增成员生效）",
     ...
     "check_method_placement_guard 钉住**判据本体**（不是轴名——只核『有没有这一条』属防线空转）："
     "通用层的成员与方法次序（L1）、**允许加在前面（含最前面）或中间**、**不得一律追加到末尾**、"
     "重载相邻、先初始化/先被用到的在前、被依赖者在前（`ObjectMapper` 在 `RestTemplate` 之前）、"
     "例外（数据转换方法跟随查询）、对「位置即次序信息、追加序不报错」这一理由、可逐条核对的五条"
     "判定标准、边界（只管新增落点、不重排既有）、存量边界（只对新增成员生效、review 对既有位置"
     "不提出问题）与依据行（含本集合取舍声明）；Java 落点只给字段/方法/重载载体并把判据回指通用层；"
    ...
    ("链式赋值顺序随字段顺序（链式调用里的赋值环节按字段顺序书写；实体类取映射表列序、其他类取成员次序，不另定一套顺序）",
     ...
     "check_chain_assignment_order_guard 钉住**判据本体**（不是轴名——只核『有没有这一条』属防线空转）："
     "通用层的赋值顺序随字段顺序（L1）、**字段顺序不另定**（回指既有两条、不得自成第二个次序来源）、"
     "赋值再排一个序即同一份字段清单的第二处呈现这一理由、可逐条核对的三条判定标准、语义例外"
     "（覆盖赋值与依赖前值；部分赋值按相对先后核对）、边界（只规定书写次序、不改求值语义、不要求补全字段）、"
    ...
    ("控制器方法须显式声明 HTTP 方法", "specs/stack/spring.adoc", "script/check_specs.py",
     "check_http_contract_guard",
     "check_http_contract_guard 钉住 Spring 栈「HTTP 接口（控制器与声明式客户端）」节的这一条（条文、判定标准、存量口径、依据名与取舍定性）"),
    ("声明式客户端与提供方共用同一份契约接口", "specs/stack/spring.adoc", "script/check_specs.py",
     "check_http_contract_guard",
     "check_http_contract_guard 钉住同节的契约共用条（只定义一份契约、客户端继承而提供方实现，含判定标准与依据名）"),
    ("契约的路径前缀只定义一次", "specs/stack/spring.adoc", "script/check_specs.py",
     "check_http_contract_guard",
     "check_http_contract_guard 钉住同节的契约前缀条（前缀由客户端声明与提供方映射各一处、契约方法上只写相对路径，含判定标准、存量口径与依据名）"),
