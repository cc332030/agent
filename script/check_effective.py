...
与 check-specs.py（每次改完规范必跑的完整性机械校验）不同，本脚本是**单独手动
执行**的体系化工具，回答一个 check-specs 回答不了的问题：
...
用法：
  python script/check-effective.py                 # 手动运行，输出抓手清单
  python script/check-effective.py --warn          # 有任何"无机械抓手"条款时以退出码 1 退出
...
MECHANISMS = [
    ("规范调整后须真正验证（要点防线）",       "specs/core/management.adoc",       "script/check-specs.py",      "check-specs 的 check_principle_guard 校验该条仍存在"),
    ("内部链接须用相对路径、禁根绝对",           "specs/general/doc.adoc",           "script/check-specs.py",      "check_link_refs"),
    ("规范引用文件必须真实存在",                 "specs/core/management.adoc",       "script/check-specs.py",      "check_refs_exist"),
    ("技术栈登记与文件一致",                     "AGENTS.adoc",                     "script/check-specs.py",      "check_stack_consistency"),
    ("私有工程约定不入全局规范",                 "specs/general/*.adoc",            "script/check-specs.py",      "check_forbidden_patterns"),
    ("临时产物清理脚本可用",                     "specs/core/execution.adoc",       "script/clean-tmp.py",        "存在清理脚本"),
    ("完整性校验配套测试",                       "specs/core/management.adoc",       "script/check_specs_test.py", "20+ 用例"),
    ("定义未执行核验配套测试",                   "specs/core/management.adoc",       "script/check-effective_test.py", "本工具的自测"),
...
def main(root: str | None = None) -> int:
    ...
    if "-warn" in sys.argv[1:] and by_status["no-grip"]:
