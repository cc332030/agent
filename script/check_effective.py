...
MECHANISMS = [
    ...
    ("本仓库 changelog 条目仅在用户明确要求时追加（未明确要求时一律不写、不改）",
     "AGENTS.adoc「版本与变更记录」", "script/check_specs.py",
     "check_changelog_timing_guard 钉住本仓库登记时机条（只在用户明确要求时追加／未明确要求时一律不写不改，含顺手补一条）与该条的抓手登记；『某次是否真的自行追加了条目、有没有漏记用户要求的条目』属运行时行为（提交内容与评论记录），机械无法判定，交人/子 agent 复核"),
