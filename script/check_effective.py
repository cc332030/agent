...
MECHANISMS = [
    ...
    ("精炼性：同一描述只写一处（重复面是必查项、收敛形态、与内容不减少的边界）",
     ...
     "**本条只钉要点文本仍在**——『这次到底查了几处、收敛了哪些、有没有把必要内容当重复删掉』属语义判断（diff 与汇报正文），机械无法判定，交人/子 agent 复核"),
    ...
    ("入口文档须给『隔一次会话还认得回来』的清单：安装文档路径与取规范脚本路径、副本检索路径、取回与更新方式、要跨会话保留的本项目信息（落点只有用户家目录下的一处）",
     ...
     "『某次安装是否真把项目信息写进入口文档』属运行时行为（目标项目文件），交人/子 agent 复核"),
    ("安装流程的幂等更新：用户手工编辑过的模板内容不自动改回（用户改过的以用户改过的为准）",
     ...
     "属运行时行为（目标项目文件与提交记录），交人/子 agent 复核"),
    ...
    ("需求先找参照物：调整内容（含新增）时须先检索现成可参照的业界标准/通行设计范式/本项目先例，有更优的设计用更优的、查不到须说出查证方式并标未确证", "specs/general/planning.adoc", "script/check_specs.py",
     "check_dev_flow_guard", "check_dev_flow_guard 钉住 planning.adoc「需求先找参照物（L1）」的条与三类判定标准（说不出参照物与检索动作 / 有标准可循却自造一套说法 / 有更优设计却按原口述照收且未说明理由）、必加载层的一行重申、公共片段 `baseline-and-compat` 与两个提示词题面的同口径句、PROMPTS 与 README 的登记；『本次到底查没查、查到的是不是更优』属运行时行为（检索过程与规划落盘），机械无法判定，交人/子 agent 复核"),
    ("规范收录侧须先找参照物、别自己造：提案校验须查现成业界标准与更优设计（存在比用户口述更优的设计时按更优的设计收）", "specs-project-maintainer/spec-lifecycle.adoc", "script/check_specs.py",
     "check_spec_admission_guard", "check_spec_admission_guard 钉住「新增规范的提案校验」的三处要点（先找参照物、别自己造 / 存在比用户口述更优的设计时按更优的设计收 / 检查是否已有标准与本项目条目）；『这次收到的条目是不是比用户口述更优』属语义判断，交人/子 agent 复核"),
