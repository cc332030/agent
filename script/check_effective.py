...
MECHANISMS = [
    ...
    ("Java 测试类拆分裁决（一个被测类可拆多个类，但同分类同属性须归一类、不得滥拆）", "specs/stack/java-testing.adoc", "script/check_specs.py", "check_java_test_naming 钉住规范侧三段判据（可拆声明 / 同分类同属性须归一类 / 禁止滥拆）与调度器侧同口径——「可拆但不得滥拆」是单一语义，只剩一半即被读成『每个场景一个类』或『一个被测类一个类』；「某次拆分是否真由分类驱动」属语义判断，交人/子 agent 复核"),
    ...
    ("安装取文件须有随规范分发的抓手：清单从入口自身解析、一次取全（不手拼逐条下载命令）", "AGENTS_COMMON.adoc + INSTALL.adoc", "script/check_specs.py", "check_spec_fetch_guard 钉住随规范分发的抓取脚本与其同名平台入口（`.sh`/`.bat`）存在、清单从入口的调度器登记解析（退回手工清单即回到「新增规范就漏一份」）、增量语义、落点边界校验、「不是站点首页」的判据（站点对未命中路径回落 200 + HTML，只判状态码会把 HTML 存成规范）、退出码语义、薄壳三件事与 `.bat` 纯 ASCII+CRLF，以及 INSTALL/AGENTS_COMMON/README 三处登记同步；『取回的副本是否真的与远端一致、清单是否真的取全』属运行时行为，靠实测与留证（本仓库按逐字节比对复核）"),
    ...
    ("取规范入口的解释器兜底：不把「机器上有 python3」当前提，且入口不得代为安装运行时",
     ...
     "check_interpreter_fallback_guard 钉住两个入口**按次序探测解释器、找不到就报错退出**"
     "（缺一即「没有 python 的机器上装不上规范」），以及安装文档的**处置次序**"
     "（先按本平台既有软件分发方式装一个 → 或让 AI 在能看见报错的会话里做 → 或退到远程入口地址）；"
