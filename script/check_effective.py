...
MECHANISMS = [
    ...
    ("数据访问边界：IService 成员方法只许在本子类内用、实体 Mapper 只由其对应实体的 Service 调用、跨表业务另建 BizService、调库前先判空（集合返回空集合）", "specs/stack/java.adoc", "script/check_specs.py",
     "check_orm_boundary_guard", "check_orm_boundary_guard 钉住三处判据本体（**按节取文本**：三个文件各有一个「持久化访问」节、节标题逐字相同，要点落回各自的节——整组从本节搬进同文件另一个节会被拦下；调度器的识别特征**按技术栈条那一行**核，故『绕开本条另立一条调度项』不构成齐备）：① `specs/stack/java.adoc`「持久化访问」的『成员方法只许在本 `IService`/`ServiceImpl` 子类内用（**其他子类也不行**）』『禁止面落到工具类/静态方法/Controller』『给出去路（放进该实体自己的 Service 或另建业务服务）』『违反本条不构成可以退回 `new LambdaQueryWrapper` 的许可』；② `specs/stack/spring.adoc`「分层与职责」的『跨表另建业务服务（`XxxBizService`）』『实体 Service 的职责边界』『**不禁用/不建议** + 补充性单表查询豁免』『判定标准（直接注入非本类实体的 Mapper）』『java.adoc 为唯一落点』；③ `specs/general/coding.adoc`「持久化访问」的『访问前先判空、不发起无效查询（L1）』『集合返回空集合、不返回 `null`』『就地返回』『防反用边界』与框架专名禁入，以及栈层的 Java 落点（`getById(null)`/`CList.empty()`/不手写裸 null 比较）与调度器识别特征；**存量口径（用户点名『已有的不管，也不告警』）另核**——须能读到『不视为违规/不告警/随动迁移』三者。『某个调用点算不算跨类调、某次查询算不算补充性单表查询』语义判断（见 GUARD_CHECK_LIMITS）"),
