...
MECHANISMS = [
    ...
    ("开发流程：先查现状/先调研最佳方案/先定基线（既有用例先跑通并留证）、大范围改动先确认、不得绕开既有体系另写一套、老用例不得为迁就改动而改判", "specs/core/execution.adoc + specs/general/planning.adoc + specs/general/testing.adoc", "script/check_specs.py", "check_dev_flow_guard 钉住必加载层四条底线（动手前先摸清现状与最佳方案／不得绕开既有体系另写一套／大范围改动先确认／改动前先定基线）与生命周期两节点、通用层 planning.adoc 的展开与依据、testing.adoc 的『重构后须同时满足既有用例与新用例』与『兼容性无法满足时先确认』、公共片段 `baseline-and-compat`/`compat` 与两个提示词的 include、调度器与 PROMPTS/README 登记；『某次是否真的先跑了基线、是否真的没改老用例』属运行时行为（改动记录、提交内容与测试统计），机械无法判定，交人/子 agent 复核"),
