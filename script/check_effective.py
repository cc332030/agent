...
MECHANISMS = [
    ...
    ("行尾与批处理编码按解释器分流（LF 基准、.bat/.cmd 必须 CRLF 且默认 GBK、.gitattributes/.editorconfig 固定；仓库根缺这两个落盘口即补齐）", "specs/general/encoding.adoc", "script/check_specs.py", "check_line_ending_guard 钉住编码规范的分流判据（LF 基准、`.bat`/`.cmd` CRLF 与 GBK、`core.autocrlf`/`.gitattributes`/`.editorconfig` 落盘口）与 bash/python/powershell 栈文件的行尾要求；某文件实际是否为 CRLF、批处理是否为 GBK、仓库根是否已有 `.gitattributes`/`.editorconfig` 属引用方工作区状态，本仓库不可见，靠引用方按 `git ls-files --eol` 与编码读回自检"),
